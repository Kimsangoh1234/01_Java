...
메서드(method)
 	...
 	ex) 메서드급 연기! ㅇㅇㅇ메소드급 영화 출현
 	@Setter 
 	Setter Getter alt shift s 누르고 생성
 	//생성하고 나서 임의로 값을 변경하지 말 것
 	
 	
 	//Setter : 필드에 작성한 변수를 복사해서 원본 변수는 변수 대신에 값을 변경하고 저장하는 메서드
 	...
 	생성자 : 파일명과 클래스이름과 똑같이 작성을 해줘야하고 객체의 기본 값을 설정하거나 값을 각각 변경해서 사용하는데 쓰이는 메서드
		...
		public 클래스이름(자료형 매개변수, 자료형 매개변수2){
			...
			예를 들어 회원가입을 진행할 때 필수로 이름 나이 전화번호 성별 주소는 넣어주고
			...
			void : 어떤 행동을 수행하기 위해 작성하고, 수행한다음 특별히 전달할 내용이 없을 때 사용
					void vocare 라틴어 보이다 비어있다
						특정행동을 하고 난다음에 행돋을 끝내는 것이지 값을 전달하거나 추가로 어떤 행동을 하
			...
			return : 어떤 행동을 실행하고 행동을 한 다음에 행동에 대한 결과를 전달
					...
					처음에 작성한 자료형과 마지막에 전달한 자료형이 동일해야함
					
		
					
					 public String 특정행동1(){
					 return"안녕하세요." // 특정 위에 String을 작성했으니 return 다음에 String을 작성
					 }
